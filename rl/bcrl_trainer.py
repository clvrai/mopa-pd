...
class BCRLTrainer(object):
	 def __init__(self, config):
		 ...
		 if config.expert_mode == 'mopa' or config.expert_mode == 'mopa-sota' or config.expert_mode == 'bc-stochastic-mopa':
	 ...
	 def _save_ckpt(self, ckpt_num, update_iter, env_step):
		 ...
		 # saving every 1m env. steps
		 # if ckpt_num % 1000000 == 0:
	 ...
	 def train(self):
		 ...
		 if config.expert_mode == 'bc' or config.expert_mode == 'bc-sota':
			 expert_runner = self._runner.run(every_steps=1, collect_expert_trajectories=True)
		 elif config.expert_mode == 'mopa' or config.expert_mode == 'mopa-sota' or config.expert_mode == 'bc-stochastic-mopa':
		 ...
		 init_step = 0
		 init_ep = 0
		 if config.bc_rl_sampling_mode == 'one-buffer' and (config.expert_mode == 'bc-stochastic' or config.expert_mode == 'bc-stochastic-randweights'):
			 # Pre-training phase
			 if step == 0:
				 policy_trajs_step = 0
			 ...
			 while step < config.max_global_step:
				 ...
				 if self._is_chef:
					 ...
					 if update_iter % config.ckpt_interval == 0:
						 self._save_ckpt(step, update_iter, env_step)
		 elif config.bc_rl_sampling_mode == 'two-buffers' and (config.expert_mode == 'bc-stochastic' or config.expert_mode == 'bc-stochastic-mopa'):
			 if step == 0:
				 print('Initialization phase: collecting trajectories for expert replay buffer...')
				 # collect trajectories from expert BC and store them in expert memory buffer
				 while init_step < config.expert_num_trajectories:
					 if config.expert_mode == 'bc-stochastic':
						 rollout, info = next(expert_runner)
						 self._agent.store_episode_expert(rollout)
						 init_step += 1
					 elif config.expert_mode == 'bc-stochastic-mopa':
				 ...
				 policy_trajs_step = 0
				 print('Initialization phase: collecting trajectories for policy replay buffer...')
				 # collect trajectories from policy
				 while policy_trajs_step < config.initial_policy_num_trajectories:
					 rollout, info = next(runner)
					 self._agent.store_episode(rollout)
			 ...
			 if step == 0:
				 ...
				 # collect trajectories from expert BC and store them in expert memory buffer
				 while init_step < config.expert_num_trajectories:
					 if config.expert_mode == 'bc' or config.expert_mode == 'bc-stochastic' or config.expert_mode == 'bc-sota':
						 rollout, info = next(expert_runner)
						 self._agent.store_episode_expert(rollout)
						 init_step += 1
					 elif config.expert_mode == 'mopa' or config.expert_mode == 'mopa-sota':
						 rollout, info, _ = expert_runner.run_episode(is_train=False, record=False, expert_mode=True)
						 steps_from_rollout = self.store_expert_rollout_from_mopa_rl(rollout)
						 init_step += steps_from_rollout
						 logger.info("Finished collecting one episode of expert rollout: {}".format(info))
	 ...
	 def evaluate(self):
		 ...
		 if self._config.three_hundred_eval_five_seeds:
			 config = self._config
			 seeds = [1234, 200, 500, 2320, 1800]
			 ...
			 if ep_success_total == 0:
				 print(colored("Average episode length: " + str(info["len"]), "yellow"))
			 else:
			 ...
			 # with open('ours_eef_positions.npy', 'wb') as f:
			 #     np.save(f, eefs)
